...
const pessoaJuridicaColecao = collection(db, "pessoa_juridica");
const contadorDoc = doc(db, "_counters", "pessoa_juridica_sequence");
...
export async function obterPessoasJuridicas(): Promise<PessoaJuridica[]> {
  const snapshot = await getDocs(pessoaJuridicaColecao);
  return snapshot.docs.map((doc) => {
    const data = doc.data();
    const { id, ...rest } = data as Record<string, any>;
    return { id: doc.id, ...rest } as PessoaJuridica;
  });
}

export async function criarPessoaJuridica(
  pessoa: Omit<PessoaJuridica, "id" | "nr_sequencia" | "dt_criacao" | "dt_alteracao" | "ds_usuario_criacao" | "ds_usuario_alteracao">,
  autor?: AuditAutor
): Promise<string> {
  ...
  const docRef = await addDoc(pessoaJuridicaColecao, {
    ...dados,
    nr_sequencia,
    dt_criacao: agora,
    dt_alteracao: agora,
  ...
  try {
    // registrar auditoria na subcollection pessoa_juridica/{id}/auditoria
    const auditCol = collection(db, "pessoa_juridica", docRef.id, "auditoria");
...
export async function atualizarPessoaJuridica(
  id: string,
  pessoa: Partial<Omit<PessoaJuridica, "id" | "nr_sequencia" | "dt_criacao" | "ds_usuario_criacao" | "ds_usuario_alteracao">>,
  autor?: AuditAutor
): Promise<void> {
  const docRef = doc(db, "pessoa_juridica", id);
  const snap = await getDoc(docRef);
  if (!snap.exists()) {
    return;
  }
  ...
  const { updates, removidos } = montarUpdateComRemocoes(currentData, pessoa);
  const agora = new Date().toISOString();
  const nomeAutor = autor?.usuarioNome?.trim() || '-';
  ...
  try {
    const auditCol = collection(db, "pessoa_juridica", id, "auditoria");
...
export async function excluirPessoaJuridica(id: string): Promise<void> {
  const docRef = doc(db, "pessoa_juridica", id);
