...
export interface CadastroGeralColumn {
  key: string;
  label: string;
  dataClass?: string;
}
...
export default function CadastroGeralListView<T extends { id?: string }>({
  loading,
  items,
  columns,
  formatCellValue,
  ...
  const paginatedItems = pageSize === 'all'
    ? items
    : items.slice((currentPageSafe - 1) * pageSize, currentPageSafe * pageSize);
  ...
  }, [columnOrder, loading, items.length]);
                ...
                <table ref={tableRef} className="text-sm" style={{ tableLayout: "fixed", width: frozenWidth || "100%", borderCollapse: "separate", borderSpacing: 0 }}>
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {paginatedItems.map((item) => (
                      <tr
                        key={item.id}
                        className={`cursor-[context-menu] hover:bg-[#eee] ${selectedId === item.id ? 'row-selected' : ''}`}
                        onClick={() => setSelectedId((prev) => (prev === item.id ? null : item.id ?? null))}
                        onContextMenu={(e) => {
                          e.preventDefault();
                          setSelectedId(item.id ?? null);
                          setContextMenu({ x: e.clientX, y: e.clientY, section: 'cadastrosGerais', item: item as unknown as ContextMenuState['item'] });
                        }}
                      >
