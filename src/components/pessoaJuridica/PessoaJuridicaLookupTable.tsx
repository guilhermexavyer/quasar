...
interface PessoaJuridicaLookupTableProps {
  pessoasJuridicas: PessoaJuridica[];
  onSelect: (pessoa: PessoaJuridica) => void;
}

export default function PessoaJuridicaLookupTable({ pessoasJuridicas, onSelect }: PessoaJuridicaLookupTableProps) {
  const tableRef = useRef<HTMLTableElement>(null);
  const [selectedId, setSelectedId] = useState<string | null>(null);
  const [frozenWidth, setFrozenWidth] = useState<string | null>(null);
  const lookupColumns = useMemo(
    () => PJ_COLUMNS.filter((col) => col.key !== 'dt_criacao' && col.key !== 'dt_alteracao'),
    []
  );
  const [columnOrder, setColumnOrder] = useState<number[]>(() =>
    Array.from({ length: lookupColumns.length }, (_, i) => i)
  );
  ...
  const sortedPessoasJuridicas = useMemo(() => {
    if (sortColumn === null || sortAsc === null) return pessoasJuridicas;
    const key = lookupColumns[sortColumn].key;
    return [...pessoasJuridicas].sort((a, b) => {
      const valA = a[key];
      const valB = b[key];
  ...
  }, [pessoasJuridicas, sortColumn, sortAsc]);
  ...
  }, [columnOrder, pessoasJuridicas.length]);
          ...
          <div className="overflow-x-auto overflow-y-auto relative flex-1 min-h-0 h-full">
            <table ref={tableRef} className="text-sm" style={{ tableLayout: "fixed", width: frozenWidth || "100%", borderCollapse: "separate", borderSpacing: 0 }}>
              ...
              <tbody className="divide-y divide-gray-200">
                {sortedPessoasJuridicas.map((pessoa) => (
                  <tr
                    key={pessoa.id}
                  ...
                  >
                    {columnOrder.map((logicalIdx) => {
                      ...
                      const displayValue = formatCellValuePj(col.key, value);
                      return (
