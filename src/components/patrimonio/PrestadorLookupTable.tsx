...
interface PrestadorLookupTableProps {
  prestadores: Colaborador[];
...
const LOOKUP_COLUMNS: LookupColumn[] = [
  { key: "nr_sequencia", label: "#", dataClass: "text-center" },
  { key: "ds_nome", label: "Nome" },
  { key: "nr_cpf", label: "CPF" },
  { key: "ds_razao_social", label: "Razão social" },
...
function formatCnpj(value: unknown): string {
  if (!value) return "";
  const str = String(value).replace(/\D/g, "");
  if (str.length !== 14) return String(value);
  return `${str.slice(0, 2)}.${str.slice(2, 5)}.${str.slice(5, 8)}/${str.slice(8, 12)}-${str.slice(12)}`;
}

export default function PrestadorLookupTable({ prestadores, pessoasFisicas, pessoasJuridicas, onSelect }: PrestadorLookupTableProps) {
  const tableRef = useRef<HTMLTableElement>(null);
  const [selectedId, setSelectedId] = useState<string | null>(null);
  const [frozenWidth, setFrozenWidth] = useState<string | null>(null);
  const lookupColumns = useMemo(() => LOOKUP_COLUMNS, []);
  ...
  function measureAllMinWidths() {
    ...
    const widths = new Array(lookupColumns.length).fill(0);
  ...
  function applyColumnWidth(table: HTMLTableElement, ths: NodeListOf<HTMLElement>, domIdx: number, width: number) {
    const th = ths[domIdx];
  ...
  function handleSortClick(logicalIndex: number) {
    if (sortColumn === logicalIndex) {
      ...
      else if (sortAsc === false) { setSortColumn(null); setSortAsc(null); }
  ...
  function handleResizeStart(logicalIdx: number, e: ReactMouseEvent<HTMLDivElement>) {
    ...
    function onMouseUp() {
      ...
      if (table) setFrozenWidth(table.style.width);
    }
  ...
  /** Enriches the prestador list with pessoa_fisica data for sorting/display. */
  const enrichedData = useMemo(() => {
    return prestadores.map((col) => {
      const pf = pessoasFisicas.find((p) => p.nr_sequencia === col.nr_seq_pessoa_fisica);
      const pj = col.nr_seq_pessoa_juridica ? pessoasJuridicas.find((p) => p.nr_sequencia === col.nr_seq_pessoa_juridica) : undefined;
      return {
        _colaborador: col,
        nr_sequencia: col.nr_sequencia,
        ds_nome: pf?.ds_nome ?? "",
        nr_cpf: pf?.nr_cpf ?? "",
        ds_razao_social: pj?.ds_razao_social ?? "",
        nr_cnpj: pj?.nr_cnpj ?? "",
      };
    });
  }, [prestadores, pessoasFisicas, pessoasJuridicas]);

  const sortedData = useMemo(() => {
    if (sortColumn === null || sortAsc === null) return enrichedData;
    const key = lookupColumns[sortColumn].key;
  ...
  }, [enrichedData, sortColumn, sortAsc]);
  ...
  }, [columnOrder, prestadores.length]);
          ...
          <div className="overflow-x-auto overflow-y-auto relative flex-1 min-h-0 h-full">
            <table ref={tableRef} className="text-sm" style={{ tableLayout: "fixed", width: frozenWidth || "100%", borderCollapse: "separate", borderSpacing: 0 }}>
              <thead>
                <tr className="bg-[#bbb]">
                  {columnOrder.map((logicalIdx, visualIdx) => {
                    const col = lookupColumns[logicalIdx];
              ...
              <tbody className="divide-y divide-gray-200">
                {sortedData.map((row) => (
                  <tr
                    key={row._colaborador.id ?? row._colaborador.nr_sequencia}
                    className={`cursor-pointer hover:bg-[#eee] ${selectedId === (row._colaborador.id ?? null) ? "row-selected" : ""}`}
                    onDoubleClick={() => {
                      setSelectedId(row._colaborador.id ?? null);
                      onSelect(row._colaborador);
                    }}
                  >
                    {columnOrder.map((logicalIdx) => {
                      const col = lookupColumns[logicalIdx];
                      const value = (row as Record<string, unknown>)[col.key];
                      const displayValue = col.key === "nr_cpf" ? formatCpf(value) : col.key === "nr_cnpj" ? formatCnpj(value) : (value !== null && value !== undefined ? String(value) : "");
                      return (
                        <td key={logicalIdx} className={`px-[10px] py-[3px] min-w-0 align-middle font-normal ${col.dataClass || ""}`} style={{ color: "#333", borderBottom: "0.5px solid rgba(0,0,0,0.06)" }}>
