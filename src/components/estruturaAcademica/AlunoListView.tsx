...
import { ALUNO_COLUMNS as COLUMNS, formatCellValue } from "@/lib/alunoUtils";
...
export default function AlunoListView({
  ...
  const totalRecords = alunos.length;
  ...
  const paginatedAlunos = pageSize === 'all'
    ? alunos
    : alunos.slice((currentPageSafe - 1) * pageSize, currentPageSafe * pageSize);
                ...
                <table ref={tableRef} className="text-sm" style={{ tableLayout: "fixed", width: frozenWidth || "100%", borderCollapse: "separate", borderSpacing: 0 }}>
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {paginatedAlunos.map((aluno) => (
                      <tr
                        key={aluno.id}
                        className={`cursor-[context-menu] hover:bg-[#eee] ${selectedId === aluno.id ? 'row-selected' : ''}`}
                        onClick={() => setSelectedId((prev) => (prev === aluno.id ? null : aluno.id ?? null))}
                        onContextMenu={(e) => {
                          e.preventDefault();
                          setSelectedId(aluno.id ?? null);
                          setContextMenu({ x: e.clientX, y: e.clientY, section: 'estruturaAcademica', item: aluno });
                        }}
                      >
