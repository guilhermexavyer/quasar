...
interface ListViewProps {
  ...
  columnLookups?: Partial<Record<keyof Colaborador, Record<number, string>>>;
}

export default function ColaboradorListView({
  message,
  loading,
  colaboradores,
  openNewForm,
  ...
  const paginatedColaboradores = pageSize === 'all'
    ? colaboradores
    : colaboradores.slice((currentPageSafe - 1) * pageSize, currentPageSafe * pageSize);
  ...
  }, [columnOrder, loading, colaboradores.length]);
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center flex-1 py-16 text-center">
              ...
              <p className="mt-1 text-sm text-slate-500">
                Clique em "Adicionar" para cadastrar um colaborador.
              </p>
            </div>
          ) : (
                ...
                <table ref={tableRef} className="text-sm" style={{ tableLayout: "fixed", width: frozenWidth || "100%", borderCollapse: "separate", borderSpacing: 0 }}>
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {paginatedColaboradores.map((colaborador) => (
                      <tr
                        key={colaborador.id}
                        className={`cursor-[context-menu] hover:bg-[#eee] ${selectedId === colaborador.id ? 'row-selected' : ''}`}
                        onClick={() => setSelectedId((prev) => (prev === colaborador.id ? null : colaborador.id ?? null))}
                        onContextMenu={(e) => {
                          e.preventDefault();
                          setSelectedId(colaborador.id ?? null);
                          setContextMenu({ x: e.clientX, y: e.clientY, section: 'estruturaAcademica', item: colaborador });
                        }}
                      >
                        {columnOrder.map((logicalIdx) => {
                          const col = COLUMNS[logicalIdx];
                          const value = colaborador[col.key];
